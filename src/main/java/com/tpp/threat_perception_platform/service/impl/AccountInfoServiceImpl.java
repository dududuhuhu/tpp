...
public class AccountInfoServiceImpl implements AccountInfoService {
    ...
    public int analyzeAndSaveAccountInfo(AccountInfo accountInfo,Date now) {
        // 检查数据库中是否已存在该账号
        AccountInfo existingAccount = accountInfoMapper.selectBySidAndMac(accountInfo.getSid(),accountInfo.getMac()); // 假设`name`是唯一标识

        if (existingAccount != null) {
            // 如果账号已存在，仅更新`updated_at`字段
            accountInfo.setId(existingAccount.getId()); // 设置ID，以便进行更新操作
