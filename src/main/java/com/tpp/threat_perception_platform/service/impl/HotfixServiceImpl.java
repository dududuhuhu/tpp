...
//    }
    ...
    @Override
    public ResponseResult analyzeAndSaveHotfixRiskReport(List<Hotfix> hotfixes, String mac) {
        ...
        try {
            ...
            // 1. 过滤出当前 mac 对应的补丁列表
            List<Hotfix> filteredHotfixes = hotfixes.stream()
                    .filter(h -> mac.equalsIgnoreCase(h.getMac()))
                    .collect(Collectors.toList());
            System.out.println("filteredHotfixes:"+filteredHotfixes);
            ...
            if (needAI) {
                String promptTemplate =
                        "你是专业的操作系统安全分析专家。以下是某台主机已安装的部分系统补丁（仅提供补丁编号和主机 MAC 地址）。请根据你掌握的知识，判断是否存在已知的高危补丁或潜在风险，并输出以下格式的 JSON 分析报告：\n\n补丁信息如下：\n%s\n\n返回格式：\n" +
                                "{\n" +
                                "  \"risk_summary\": \"整体风险判断（如：无风险/可能存在高危风险）\",\n" +
                                "  \"reason\": \"你判断的主要原因或可能对应的历史漏洞\",\n" +
                                "  \"consequence\": \"若存在风险，可能造成的影响\",\n" +
                                "  \"suggestion\": \"安全建议，如是否需要卸载或进一步分析\"\n" +
                                "}\n\n" +
                                "⚠️ 注意：请只返回标准 JSON，不要包含代码块符号或解释文字。";
    ...
    private String buildHotfixInfoForPrompt(List<Hotfix> hotfixes) {
    ...
    private String buildHotfixSummaryContent(List<Hotfix> hotfixes) {
        hotfixes.sort(Comparator.comparing(Hotfix::getHotfixId));
        StringBuilder sb = new StringBuilder();
        for (Hotfix hotfix : hotfixes) {
