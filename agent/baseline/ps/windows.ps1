...
function Write-FullOutput {
    ...
    if ($Object -is [hashtable]) {
        foreach ($key in $Object.Keys) {
            Add-Content -Path $FilePath -Value "键: $key"
            Add-Content -Path $FilePath -Value "值: $($Object[$key])"
            Add-Content -Path $FilePath -Value "---"
        }
    } else {
        $Object | Format-List * | Out-String -Width 4096 | Add-Content -Path $FilePath
    }
}
...
Write-FullOutput -Object $CollectKB -FilePath $file_name

echo "————————————————————————" >> $file_name
echo "当前运行端口信息一览" >> $file_name
F_Logging -Level Info -Msg "[-] 当前运行端口信息一览"
netstat -ano | Out-String -Width 4096 >> $file_name

echo "————————————————————————" >> $file_name
echo "系统服务运行一览" >> $file_name
F_Logging -Level Info -Msg "[-] 系统服务运行一览"
$ServiceCheck = F-Service
Write-FullOutput -Object $ServiceCheck -FilePath $file_name

echo "————————————————————————" >> $file_name
echo "当前系统账户信息一览" >> $file_name
F_Logging -Level Info -Msg "[-] 当前系统账户信息一览"
$SysAccount = F_SysAccount
Write-FullOutput -Object $SysAccount -FilePath $file_name

echo "————————————————————————" >> $file_name
echo "系统中所有的用户信息一览" >> $file_name
F_Logging -Level Info -Msg "[-] 系统中所有的用户信息一览"
$OtherUserCheck = F_OtherUserPolicy
Write-FullOutput -Object $OtherUserCheck -FilePath $file_name

echo "————————————————————————" >> $file_name
echo "当前系统安全策略信息一览" >> $file_name
F_Logging -Level Info -Msg "[-] 当前系统安全策略信息一览"

$SysAccountPolicy = F_SysAccountPolicy
$SysEventAuditPolicy = F_SysEventAuditPolicy
$SysUserPrivilegePolicy = F_SysUserPrivilegePolicy
$SysSecurityOptionPolicy = F_SysSecurityOptionPolicy
$SysRegistryPolicy = F_SysRegistryPolicy
$SysProcessServicePolicy = F_SysProcessServicePolicy

"SysAccountPolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysAccountPolicy -FilePath $file_name

"SysEventAuditPolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysEventAuditPolicy -FilePath $file_name

"SysUserPrivilegePolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysUserPrivilegePolicy -FilePath $file_name

"SysSecurityOptionPolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysSecurityOptionPolicy -FilePath $file_name

"SysRegistryPolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysRegistryPolicy -FilePath $file_name

"SysProcessServicePolicy 检查结果：" >> $file_name
Write-FullOutput -Object $SysProcessServicePolicy -FilePath $file_name

echo "————————————————————————" >> $file_name
echo "当前系统杂类信息一览" >> $file_name
F_Logging -Level Info -Msg "[-] 当前系统杂类信息一览"
$OtherCheck = F_OtherCheckPolicy
Write-FullOutput -Object $OtherCheck -FilePath $file_name
